...
| Skills | `options.settingSources` + `"Skill"` in allowedTools | config 开关 | `skills.sdkNative` |
...
- 配置中新增 `skills.sdkNative` 开关控制是否启用 SDK 原生 Skills
- 默认启用
...
skills?: {
  ...
  sdkNative?: boolean;         // SDK 原生 Skills 开关（新增，默认 true）
};
```

### 5.5 实现要点

- `runtime/sdk.ts` — 当 `skills.sdkNative` 为 true 时，加入 `settingSources: ["user", "project"]`
- `runtime/tool-policy.ts` — 当 `skills.sdkNative` 为 true 时，`allowedTools` 加入 `"Skill"`
- Phase 1 的 `skills.ts` 保持不变，继续作为 prompt append 使用
- 用户在 `<cwd>/.claude/skills/` 下放置 SKILL.md 即可被 SDK 发现
...
const skillsConfigSchema = z.object({
  ...
  sdkNative: z.boolean().default(true)          // 新增
}).strict();
